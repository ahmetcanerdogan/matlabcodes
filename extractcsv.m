...
trialnumber = input(3);
 ...
 filenamin = ['S' ,num2str(subject), 'C' ,num2str(casenumber), 'T',num2str(trialnumber) ]
...
%             repnames = strrep(repnames,'.' , '_');
            ...
            for j = 1: length(varnames)
                ...
                if isnan(str2double(dataArray{:,j}(1:min(length(dataArray{:,j}),10))))
                    eval([topicname '.' namesegs '= dataArray{:,j};']);
                else
                    eval([topicname '.' namesegs '= str2double(dataArray{:,j});']);
